...
export class IlsInfoIndicator extends DisplayComponent<IlsInfoIndicatorProps> {
    ...
    render(): VNode | null {
        return (
            <Layer x={748} y={28}>
                <text x={-102} y={0} fontSize={25} class="White" textAnchor="end">ILS1</text>
                ...
                <text x={20} y={30} fontSize={25} class="Cyan" textAnchor="end">
                    {this.courseTextSub}
                    &deg;
                </text>

                <g visibility={this.ilsFrequency.map((v) => v > 0).map(this.visibilityFn)}>
                    <text x={-80} y={58} fontSize={20} class="White" textAnchor="end" textDecoration="underline">
                        {this.tuningModeTextSub}
                    </text>
                </g>

                <text x={0} y={60} fontSize={25} class="White" textAnchor="end">
                    {this.ilsIdent}
                </text>
            </Layer>
        );
